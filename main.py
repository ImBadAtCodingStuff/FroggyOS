...
def CONTROLLER():
    if myInput.joystick.get_count() == 0:
...
autoTickThread = multiprocessing.Process(target=autonomous.autoTick)
controllerThread = multiprocessing.Process(target=CONTROLLER)
#listen_thread.start()
...
while True:
    ...
    if len(buf) > 0:
        ...
        if decoded == 'mode = controller':
            if existing_auto >= 1:
                print("\n\n\cancel auto\n\n")
                print("please wait while canceling auto....")
                Global_var.continue_auto = 'false'
                sleep(2)
                autoTickThread.terminate()
                existing_auto = 0
                sleep(2)
        ...
        if decoded == 'mode = autonomous':
            Global_var.continue_auto = 'true'
            if existing_controller >= 1:
                print("\n\n\cancel controller\n\n")
                print("please wait while canceling controller....")
